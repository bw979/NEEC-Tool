...
compute_Plasma_Rate_pmap <- function(ne, Te_keV, Eres_keV, S_neec_beV, CS, Z) {
  #Te
  ...
  fq <- CSD_neTe$frac[which(CSD_neTe$q==(CS+1))]

  ne * MB(Eres_keV*1000, 1E3*Te_keV) *  sqrt(1 - (1/((1+(Eres_keV*1000) / me))^2))*100*c   *  S_neec_beV * 1E-24 * (2/(pi)) *  fq
}
...
#beV, keV, eV                                         ###Set as "Te"
compute_Plasma_Rate_pmap <- function(Wins_all, tau, Te_input, ne_input, OUTPUT, OUTPUT2) {
...
#### MAY NEED UNCOMMENTING ####  
  ...
  R_neec <- phi_res * E_frac * Sexp * (2/pi) 
