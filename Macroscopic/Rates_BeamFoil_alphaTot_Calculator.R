...
################################################################################
## FUNCTION THAT CALCULATES THE UPPER LIMIT P_NEEC USING alpha_Tot #############
################################################################################
                                                     #SI, keV, keV          , keV/um
P_NEEC_alphaTot <- function(Jf, Jat_mean, Ji, ICC_tot, Ar, Q, Ebind_mean, M, dedx) {
  ## Beam Energy Explored
  # Read the correct E,dedx array... parse the element onto the chosen target for which there is SRIM data
  dedx_read <- read_csv("Dependencies/Beam_Data/Mo_C_dEdx.csv")
  
  Ee <- Q - Ebind_mean
  
  #Effective beam resonance energy
  Ebeam_eff <- mp.me * Ee * 1E-3 * 84   # MeV
  
  #Ebeam_eff <- Q - Ebind_mean
  
  ## Must find appropriate thickness based on range
  Thickness <- 100 #um
  stepsize <- 0.001
  nsteps <- (Thickness/stepsize) + 1
  
  ## Entrance Beam Energy
  Ebeam_MeV <- Ebeam_eff #MeV
  
  ### bulk values ... currently set at carbon
  ...
  E_val <- Ebeam_eff # MeV
  ### What is the effective Ion Energy?
  ...
  ## Output
  P <- ne * compute_S(Jf, Jat_mean, Ji, ICC_tot, Ar, Q, Ebind_mean *1E-24 * 1E8 * 1E-6 * M * mp.me * 1/(dedx_res_alphaTot))
return(P)
}
...
### Read the correct Energy loss array ... ENERGIES ARE ALWAYS IN MeV and MeV/um
