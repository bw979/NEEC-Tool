...
# ### In ICC multipliers sort out a Jat column
# ICC_Multipliers <- mutate(ICC_Multipliers, Jat = double(1))
# 
# Jat_compute <- function(Z_number, n_in){
#   Ebind_level <- filter(Ebinds, Z==Z_number) %>% filter(Occ==n_in)
#   # if(OUTPUT_TYPE == "Configuration") return(Ebind_level$Ground_Config)
#   # if(OUTPUT_TYPE == "IE") return(Ebind_level$IE)
#   ## Assign correct Jat
#   ## Remove numbers from string, then extract the last letter
#   s <- str_remove_all(Ebind_level$Ground_Shells, "\\d")
#   last <- substr(s,nchar(s), nchar(s))
#   l <- switch(last, "s" = 0, "p" = 1, "d" = 2, "f" = 3, "g" = 4 )
#   Jat <- l + 0.5
#   #if(OUTPUT_TYPE == "Jat") return(Jat)
#   return(as.double(Jat))
# }
# 
# ICC_Multipliers$Jat <- map2(ICC_Multipliers$Z, ICC_Multipliers$Occ, Jat_compute)
# ICC_Multipliers$Jat <- as.double(ICC_Multipliers$Jat)
# #ICC_Multipliers$Jat <- Jat_compute(ICC_Multipliers$Z, ICC_Multipliers$Occ[i])
...
Wap <- mutate(Wap, Ji_double=double(1), Jf_double=double(1), MIX_type = character(1), Gamma_Thalf_eV=double(1), Gamma_eV=double(1), 
              ICC_tot = double(1), ICC_sum = double(1), ICC_p=double(1), Stot=double(1), Rate_Pl_tot=double(1), Rate_EBIT_tot=double(1), Ebind_mean=double(1), Jat_mean=double(1), Sp=double(1)) 

Wap <- select(Wap, AX, M, El, Z, N, Ei, Ef, Ji, Ji_double, Ji_double, Jf, Jf_double, Type, Thi, Thi_numeric, Thf, Thf_numeric,
              ...
              ICC_p, ICC_tot, -ICC_sum, Sp, Stot, Ee, -Rate, Rate_Pl_tot, Rate_EBIT_tot) %>%
  filter(Type != "E0")
...
#  if( !is.na(Wap$Btype[i]) && !is.na(Wap$B2type[i]) && Wap$Btype[i] =="BM1W" && Wap$B2type[i]=="BE2W=" && Wap$MR[i]!=0 && !is.na(Wap$MR[i])  ){
  ...
  if(   !is.na(Wap$Ji_double[i]) && !is.na(Wap$Jf_double[i]) && !is.na(Wap$Type[i]) && Wap$Type[i] != "E0" && Wap$Type[i] != "Large_L") {
    ...
    # } else {
      ...
      try(Wap$ICC_p[i] <- Compute_ICC_singles(Wap$Type[i], Wap$subshell[i], Wap$Z[i], Wap$Q[i], Wap$Occ[i], "p", FALSE, 1, ""))
...
Wins_all_location_save <- "Wap_AllQ_PlasmaEBIT_NoMixing_CorrectedSubmission2022.csv"
